...
TESTS {
	...
	q = pdu_make("PUT.SAMPLE", 3,
		"1002", /* new window! */
		"sample1", "11");
	is_int(pdu_send_and_free(q, kernel), 0, "sent PUT.SAMPLE for t1002 to kernel");
	CHECK(a = pdu_recv(kernel), "failed to get reply from kernel");
	...
	q = pdu_make("PUT.COUNTER", 3,
			"2002", /* new time window */
			"counter1", "1");
	is_int(pdu_send_and_free(q, kernel), 0, "sent PUT.COUNTER for t2002 to kernel");
	CHECK(a = pdu_recv(kernel), "failed to get reply from kernel");
	is(pdu_type(a), "OK", "got [OK] from kernel");
