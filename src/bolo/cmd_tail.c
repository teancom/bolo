...
int cmd_tail(int off, int argc, char **argv)
{
	OPTIONS.verbose   = 0;
	...
	optind = ++off;
	for (;;) {
		int c = getopt_long(argc, argv, "h?Vv+e:m:TRACES", long_opts, &off);
		if (c == -1) break;
		...
		case '?':
			printf("bolo v%s\n", BOLO_VERSION);
			printf("Usage: bolo tail [-h?Vv] [-e tcp://host:port] [-TRACES] [-m PATTERN]\n\n");
			printf("Options:\n");
