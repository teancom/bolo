...
{
	if (len != NSCA_PACKET_LEN) return NULL;

	nsca_t npkt;
	memcpy(&npkt, buf, len);

	size_t len_name   = strlen(npkt.host) + 1 + strlen(npkt.service);
	size_t len_summary = strlen(npkt.output);

	bolo_t *pkt = malloc(MIN_BOLO_LEN + len_name + len_summary);
	pkt->version   = 1;
	pkt->timestamp = npkt.timestamp;
	pkt->status    = ntohs(npkt.status) & 0xffL;
	snprintf(pkt->payload, len_name+1, "%s:%s", npkt.host, npkt.service);
	snprintf(pkt->payload + len_name+1, len_summary+1, "%s", npkt.output);
