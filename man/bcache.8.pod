...
bcache - Bolo Caching Daemon

=head1 SYNOPSIS

B<bcache> [options] -e tcp://bolo:2997 -B tcp://127.0.0.1:2997

=head1 DESCRIPTION

#INTRO

B<bcache> is a special bit of bolo infrastructure that subscribes to an upstream
...
By default, B<bcache> will fork into the background, detach its terminal
...
Specify where B<bcache> should write its PID to, for control by init
scripts.  Defaults to B</var/run/bcache.pid>.  Has no effect if B<-F> is
given.
...
User and group to drop privileges to.  By default, B<bcache> will run as
...
you can transparently front it with B<bcache> broadcasting on I<tcp://*:2997>
