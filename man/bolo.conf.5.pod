...
data (via KEY statements through B<send_bolo>(1)) will be written there.

=item B<dumpfiles> /var/tmp/mon.%s

When B<stat_bolo>(1) initiates a DUMP to get all of the state, event and
...
Like states, samples and counters must be defined in order
