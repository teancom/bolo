...
bolo - Bolo Monitoring System Daemon

=head1 SYNOPSIS

B<bolo> [options]

B<bolo> -c /path/to/bolo.conf
...
Normally, B<bolo> forks into the background and detaches from the
controlling terminal, so that it can run as a system service.  This flag
disabled that behavior.  As a side effect, all logging will be forced to go
...
Defaults configuration file that B<bolo> reads on start up.
See B<bolo.conf>(5) for details.

=item B</var/run/bolo.pid>

In daemon mode (the default, unless B<-F> is given), B<bolo> will write its
process ID to this file, so that other programs (like B</etc/init.d/bolo>)
can determine if the B<bolo> process is alive, and send signals to it.

This can be changed via B<bolo.conf>(5).

=item B</var/lib/bolo/save.db>

B<bolo> will periodically write everything it knows about states, events,
...
B<bolo> binds to to three separate endpoints when it starts up.
...
and devices.  B<bolo> cannot be queried via the listener endpoint.
...
allows clients like B<stat_bolo>(1) to query specific state and event
